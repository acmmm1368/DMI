...
class DMI(nn.Module):
    ...
    def forward(self, x, lens=None, norm=True):
        ...
        temperature = self.temperature  # 0.1 0.08
        min_mi_loss = 0
        max_mi_loss = 0
        cnt_min = 0
        ...
        for j in range(1, self.num_layer):
            for i in range(self.num_layer):
                if num_rec[i] < self.num_layer - 1 and num_rec[(i + j) % self.num_layer] < self.num_layer - 1:
                    min_mi_loss += kl_div(self.softmax(out[i] / temperature).log(),
        ...
        if self.training:
            return out, down_embedding, input_embedding, max_mi_loss, min_mi_loss, final_out
        return final_out
