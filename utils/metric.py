...
class MetricLogger(object):
    def __init__(self, logger, delimiter="\t", device=torch.device('cuda')):
    ...
    def add_meters_scalar(self, tb_writer_dict):
        scalar = {}
        for name, meter in self.meters.items():
            scalar[name] = meter.value
        tb_writer_dict["writer"].add_scalars("loss_acc", scalar, tb_writer_dict["global_steps"])
